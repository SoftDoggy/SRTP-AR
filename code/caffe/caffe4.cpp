...
int main(int argc, char** argv) {
	...
	//std::cout <<"请输需要读取的目录文件名路径：" << std::endl;
	string folder ="D:\\OneDrive\\pic\\srtpar";
	
	int sum = 0;
	do {
		vector<string> files;
		getFiles(folder, "jpg", files);
		if (sum != files.size()) {
			Sleep(1000);
			...
			cv::Mat img = cv::imread(files[files.size()-1], -1);
			CHECK(!img.empty()) << "Unable to decode image " << files[files.size() - 1];
			std::vector<Prediction> predictions = classifier.Classify(img);
			Prediction p = predictions[0];
			std::cout << std::fixed << std::setprecision(4) << p.second << " - \""
				<< p.first << "\"" << std::endl;
			fout << files.size() << std::endl;
			fout << std::fixed << std::setprecision(4) << p.second << " - \""
				<< p.first << "\"" << std::endl;
